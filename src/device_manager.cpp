...
DeviceManager::DeviceManager(QObject *parent): QObject(parent), m_client(mqtt_address) {
    try {
...
void DeviceManager::connect_to_broker() {
    try {
        mqtt::connect_options opts = mqtt::connect_options_builder().clean_session().automatic_reconnect().finalize();
